...
var k_pos = [4,2]; //white kings position

if (firstmove== false){

b.cell([4,2]).DOM().classList.add("notclickable") 
}
//disables the kings to move for the first move only
...
for (var i=0; i<pieces.length; i++){
	if (pieces[i].textContent == "WP" || pieces[i].textContent == "WK"){
...
let socket = io.connect(window.origin +"/white")
...
socket.on("bm",(data)=>{
movePieceSocket(data[0],data[1])}) //gets the move from the black player
...
function movePiece() {
    ...
    if (bindMoveLocs.indexOf(userClick)) {
		...
		unclickable_board()
			...
			document.getElementById("results").firstElementChild.innerHTML = "Black Won!";
		document.getElementById("results").style.display="block"
		unclickable_board()
  			}
      	}
      	...
      	where = userClick
      	socket.emit("wm",[from,where,room]) //sends the move to the server
        resetBoard();
        highlight();
        ...
        if (bindMovePiece.textContent=="WK"){k_pos=where}
...
function highlight() {
	...
	firstmove = true; //sets the firstmove to true
	socket.emit("cbm",[1,room])

unclickable_board() // after making the move it makes the board unclickable for the player
...
function highlightSocket(from_s,where_s){
	...
	firstmove = true; //sets the firstmove to true
if (firstmove){
b.cell([0,2]).DOM().classList.remove("notclickable");
b.cell([4,2]).DOM().classList.remove("notclickable");
//enables the kings to move
...
function checkmate(r,c){
	...
	if (score==0){
		unclickable_board()
		socket.emit("result",["White is Checkmated. You Win!",room])
